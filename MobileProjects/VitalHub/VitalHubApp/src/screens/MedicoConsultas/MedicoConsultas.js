    ...
    return (
        <>
            ...
            <Container>
                ...
                <FlatList
                data={Consultas}
                renderItem={({item}) => <CardConsulta tipoCard={item.status} status={item.status == statusLista ?? false}  abrirModal={item.abrirModal} nomePaciente={item.nomePaciente} caminhoImage={item.caminhoImage} idadePaciente={item.idadePaciente} tipoConsulta={item.tipoConsulta} horaConsulta={item.horaConsulta} />}
